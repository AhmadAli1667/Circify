const coreMovies = [
  // Array of 55 hand-curated movie objects with real data
  // These are the "premium" entries with accurate info, real posters, and proper metadata
  {
    title: 'The Shawshank Redemption',
    year: 1994,
    genre: ['Drama'],
    // Single-genre array — used for filtering and recommendations
    director: 'Frank Darabont',
    leadCast: ['Tim Robbins', 'Morgan Freeman'],
    // Array of actor names — must match actor.name in actors.js to create clickable links
    rating: 9.3,
    // Default rating on a 0–10 scale — can be overridden by user's personal rating
    synopsis:
    ...
    relatedTitle: null
    // null means no related prequel/sequel; a string title links to another movie in this file
  },
  {
    title: 'The Godfather',
    year: 1972,
    ...
    // Links this movie to its sequel — used by getSmartRecs to give +3 score bonus
  },
  {
    ...
    // Links back to The Godfather — bidirectional relation
  },
  {
  ...
  },
  {
    ...
    // Anne Hathaway is in actors.js so her name becomes a clickable link
    rating: 8.7,
    synopsis:
  ...
  },
  {
    ...
    leadCast: ['Robert Pattinson', 'Zoe Kravitz'],
    rating: 7.8,
    synopsis:
      'In his second year as Gotham vigilante, Batman uncovers a conspiracy tied to the citys political elite.',
    relatedTitle: null
  },
  {
    title: 'The Holdovers',
    ...
    leadCast: ['Paul Giamatti', 'Da Vine Joy Randolph'],
    rating: 8.0,
    synopsis:
      'A strict teacher, a grieving cook, and a stranded student form an unexpected bond over a lonely winter break.',
    relatedTitle: null
  },
  {
    title: 'Poor Things',
    ...
    leadCast: ['Emma Stone', 'Mark Ruffalo'],
    rating: 7.8,
    synopsis:
      'A young woman brought back to life embarks on a radical journey of self-discovery and social rebellion.',
    relatedTitle: null
  },
  {
    title: 'The Substance',
    ...
    leadCast: ['Demi Moore', 'Margaret Qualley'],
    rating: 7.6,
    synopsis:
      'A fading star experiments with a black-market treatment promising youth, leading to terrifying consequences.',
    relatedTitle: null
  },
  {
    title: 'Civil War',
    ...
    leadCast: ['Kirsten Dunst', 'Cailee Spaeny'],
    rating: 7.2,
    synopsis:
      'Journalists travel across a fractured United States to document conflict before the capital falls.',
    relatedTitle: null
  },
  {
    title: 'Past Lives',
    ...
    leadCast: ['Greta Lee', 'Teo Yoo'],
    rating: 7.9,
    synopsis:
      'Childhood friends reconnect decades later and confront how time, identity, and migration shaped their lives.',
    relatedTitle: null
  },
  {
    ...
    leadCast: ['Ryunosuke Kamiki', 'Minami Hamabe'],
    rating: 8.2,
    synopsis:
      'Postwar Japan faces a catastrophic new threat as survivors struggle with guilt, duty, and rebuilding.',
    relatedTitle: null
  },
  {
    ...
    leadCast: ['Sandra Huller', 'Swann Arlaud'],
    rating: 7.8,
    synopsis:
      'A writer is put on trial after her husbands death, and the courtroom dissects truth, marriage, and perception.',
    relatedTitle: null
  }
]

const extendedTitles = [
  // Array of 68 movie title strings used to procedurally generate additional movies
  // These fill out the library — directors, cast, and ratings are assigned algorithmically below
  'The Pianist',
  'The Lion King',
  'The Usual Suspects',
  'Schindlers List',
...
const directorPool = [
  // 10 director names cycled through for the extended movies using modulo indexing
  'Steven Spielberg',
  'Christopher Nolan',
  'Martin Scorsese',
  'Denis Villeneuve',
...
const castPool = [
  // 10 actor pairs cycled through for the extended movies
  // Each entry is a 2-element array that becomes the leadCast
...
const genrePool = [
  // 7 genre combinations cycled through for extended movies that don't match keyword rules
  ['Drama', 'Thriller'],
  ['Crime', 'Drama'],
  ['Action', 'Adventure'],
  ['Sci-Fi', 'Drama'],
...
const inferGenres = (title, index) => {
  // Smart genre assignment based on keywords in the movie title
  // Falls back to the genrePool cycle for titles without recognizable genre keywords

  const lower = title.toLowerCase()
  // Lowercase comparison so "Star Wars" matches "star wars"

  if (lower.includes('star wars') || lower.includes('dune') || lower.includes('blade runner')) {
    return ['Sci-Fi', 'Adventure']
    // Space operas and sci-fi franchises
  }
  if (lower.includes('mission') || lower.includes('top gun') || lower.includes('fallout')) {
    return ['Action', 'Adventure']
    // Action/spy genre titles
  }
  if (lower.includes('kill bill') || lower.includes('django')) {
    return ['Action', 'Crime']
    // Tarantino revenge films
  }
  return genrePool[index % genrePool.length]
  // For all other titles: pick a genre combo based on index position in the extended list
}

const extendedMovies = extendedTitles.map((title, index) => {
  // Procedurally generates a full movie object for each extended title
  // index: the position in extendedTitles array (0–67)

  const year = 1980 + (index * 3) % 45
  // Generates years in the 1980–2024 range; cycles with a period of 45 years
  // NOTE: This assigns years algorithmically so some may not match real release years

  const rating = Number((7.4 + ((index % 21) * 0.09)).toFixed(1))
  // Generates ratings between 7.4 and 9.3 cycling every 21 movies
  // toFixed(1) ensures one decimal place; Number() converts back from string

  const director = directorPool[index % directorPool.length]
  // Cycles through the 10 directors — every 10th movie shares a director

  const leadCast = castPool[index % castPool.length]
  // Cycles through the 10 cast pairs — every 10th movie shares the same cast

  const genre = inferGenres(title, index)
  // Infers the genre from the title keywords or falls back to the genre cycle

  return {
    title,
    // The extended title string from the array above
    year,
    genre,
    director,
    leadCast,
    ...
    // Generic template synopsis — all extended movies use this formula
    relatedTitle: null
    // No prequel/sequel relations for procedurally generated movies
  }
})

const posterOverrides = {
  // Map of movie titles → real Wikipedia poster image URLs
  // Movies in this map get a real poster instead of a random picsum.photos placeholder
  ...
  // Key without quotes works because it has no spaces — JavaScript treats bare property names as strings
...
const allMoviesRaw = [...coreMovies, ...extendedMovies].map((movie, index) => {
  // Combines core (55 hand-curated) + extended (68 procedural) movies into one array
  // index is used to generate the unique numeric ID (starting at 1)

  const safeTitle = movie.title.replace(/[^a-z0-9]+/gi, '-').toLowerCase()
  // Creates a URL/seed-safe version of the title by replacing non-alphanumeric chars with hyphens
  // Used as the picsum.photos seed to generate a consistent placeholder image per movie

  return {
    id: index + 1,
    // Unique numeric ID starting from 1 (not 0) — used everywhere as the movie's key
    title: movie.title,
    genre: movie.genre,
    ...
    // Uses the real Wikipedia poster if available; otherwise generates a consistent placeholder
    // picsum.photos/seed/X/420/620 returns the same 420×620 image every time for seed X
    trailerLink: `https://www.youtube.com/results?search_query=${encodeURIComponent(`${movie.title} official trailer`)}`,
    // Links to a YouTube search results page — not a real embedded trailer
    // encodeURIComponent ensures the title is URL-safe (spaces → %20, colons → %3A, etc.)
    relatedTitle: movie.relatedTitle
    // The related movie's title string; resolved to an ID in the next processing step
  }
})

const titleToId = Object.fromEntries(allMoviesRaw.map((movie) => [movie.title, movie.id]))
// Builds a lookup map: { "The Dark Knight": 4, "Batman Begins": 5, ... }
// Used in the next step to convert relatedTitle strings into numeric IDs

const movies = allMoviesRaw.map((movie) => ({
  // Final processing pass: converts relatedTitle strings to relatedPrequelSequelId numbers
  ...
  // If relatedTitle is a non-null string, look up its ID in titleToId
  // Falls back to null if the title string doesn't match any movie in the list
}))

export default movies
// Default export of the final 136-movie array — imported by MovieContext.jsx as the seed dataset
