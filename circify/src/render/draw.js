...
export function drawGrid(ctx, viewport, width, height) {
  ...
  for (let x = startX; x <= Math.ceil(right / GRID_SIZE) * GRID_SIZE; x += GRID_SIZE) {
    ctx.moveTo(x, top);
    ctx.lineTo(x, bottom);
  }
  const startY = Math.floor(top / GRID_SIZE) * GRID_SIZE;
  for (let y = startY; y <= Math.ceil(bottom / GRID_SIZE) * GRID_SIZE; y += GRID_SIZE) {
    ctx.moveTo(left, y);
    ctx.lineTo(right, y);
  }
  ctx.stroke();
...
export function drawConnections(ctx, edges, nodes, timeMs) {
  for (const edge of edges.values()) {
    ...
    // Signal value badge at midpoint
...
export function drawNode(ctx, node, selectedNodeID) {
  ...
  const stroke = selected ? PALETTE.selected : '#2e4055';
  const glowColor = isHigh ? PALETTE.high : selected ? PALETTE.selected : '#0e1c2a';
  ...
  // Body fill — gradient from slightly lighter at top
  const grad = ctx.createLinearGradient(node.x, node.y, node.x, node.y + node.height);
  grad.addColorStop(0, 'rgba(22, 32, 48, 0.97)');
  grad.addColorStop(1, 'rgba(10, 16, 26, 0.97)');
  ctx.fillStyle = grad;
  ctx.fill();
  ...
  // Type-specific interior drawing
  switch (node.type) {
    case 'CLK':
      drawClockBody(ctx, node);
      break;
    case 'DFF':
      drawDFFBody(ctx, node);
      break;
    case 'SRLATCH':
      drawSRLatchBody(ctx, node);
      break;
    case 'INPUT': {
    ...
    case 'OUTPUT': {
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(node.outputState === SIGNAL.HIGH ? '1' : node.outputState === SIGNAL.LOW ? '0' : 'Z', cx, cy);
      ctx.restore();
      break;
    }
    default:
      // Logic gates — draw SVG icon path
      if (ICON_PATHS[node.type]) {
        ctx.save();
        ctx.translate(node.x + node.width * 0.52, node.y + node.height * 0.5);
        ctx.scale(1.15, 1.15);
        ctx.lineWidth = 1.5;
        ctx.strokeStyle = isHigh ? '#a0d8f0' : '#8aa8c0';
  ...
  // Output pin dot (skip DFF — it has its own)
  ...
  ctx.save();
  ctx.fillStyle = selected ? '#c8e8f8' : '#7a9ab2';
...
export function drawConnectPreview(ctx, connecting, snappedTarget, _viewport) {
  if (!connecting) return;

  const p0 = connecting.from;
  const p3 = connecting.to;
